...
class LangChainReActAgent:
    ...
    def __init__(self):
        ...
        self.base_tools = [self.tavily_tool, get_knowledge_list_tool, retriever_document_tool]
...
""")
    ...
    def _create_react_compatible_tool_wrapper(self, original_tool):
        ...
        async def wrapped_tool_func(tool_input: str) -> str:
            ...
            logger.info(f"工具 {original_tool.name} 收到输入: '{tool_input}' (类型: {type(tool_input)})")
            
            try:
                # 如果工具有 args_schema，优先尝试解析 JSON 输入
                if hasattr(original_tool, 'args_schema') and original_tool.args_schema:
                    logger.info(f"工具 {original_tool.name} 有 args_schema，尝试 JSON 解析")
                    try:
                        # 尝试将字符串解析为 JSON
                        ...
                        logger.warning(f"JSON 解析失败 ({json_error})，尝试直接使用字符串")
                        # 如果不是 JSON，尝试直接使用字符串
                        ...
                        try:
                            ...
                            for fallback_format in fallback_formats:
                                try:
                                    ...
                                    result = await original_tool.ainvoke(fallback_format)
                    ...
                    logger.info(f"工具 {original_tool.name} 无 args_schema，直接使用字符串")
                    result = await original_tool.ainvoke(tool_input)
                
                # 确保返回字符串格式
                result_str = str(result) if result is not None else "工具执行完成，但无返回结果"
                logger.info(f"工具 {original_tool.name} 执行成功，返回结果长度: {len(result_str)}")
                return result_str
    ...
    async def _get_all_tools(self) -> List[Any]:
        ...
        for tool in original_tools:
            try:
                ...
                logger.info(f"  有 args_schema: {hasattr(tool, 'args_schema') and tool.args_schema is not None}")
        ...
        try:
            ...
            async for event in agent_with_history.astream_events(
                ...
                # 处理工具调用 - 与原版本相同的逻辑
                    ...
                    call_id = f"call_{run_id}_{tool_name}" if run_id else f"call_{hash(str(tool_name) + str(tool_input))}"
                    ...
                    logger.info(f"ReAct Agent 工具调用: {tool_name}, call_id: {call_id}")
                    yield {
                        "type": "tool_call",
                        "data": {
                            "name": tool_name,
                ...
                # 处理工具调用结束 - 与原版本相同的逻辑
                    ...
                    mapping_key = f"{run_id}_{tool_name}" if run_id else tool_name
                    call_id = getattr(self, '_tool_call_mapping', {}).get(mapping_key, f"call_{hash(str(tool_name))}")
                    
                    logger.info(f"ReAct Agent 工具结果: {tool_name}, tool_call_id: {call_id}")
                    yield {
                        "type": "tool_result",
                        "data": {
                            "name": tool_name,
...
if __name__ == "__main__":
    ...
    async def test_react_agent():
        """测试 LangChain ReAct Agent 的基本功能"""
        session_id = "test_react_session_123"
        user_input = "你好，请帮我搜索一下最新的AI技术发展，并从知识库中查找相关资料"
        
        print("开始测试 LangChain ReAct Agent...")
        ...
        try:
            async for event in main_graph_execution(user_input, session_id):
                ...
                if event_type == "thought":
                    ...
                    print(f"🔧 工具调用: {event_data.get('name')} | 参数: {event_data.get('args')}")
                elif event_type == "tool_result":
                    print(f"📋 工具结果: {event_data.get('name')} | 内容: {event_data.get('content')[:100]}...")
